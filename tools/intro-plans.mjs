...
const SEP = 3; // min air between two pieces that are both still away from home
...
function explode() {
  ...
  // first-version cadence: 3-4 hops per piece, every piece starting on its own
  // random beat so the whole word is in motion at once
  ...
  for (let b = 0; b < 80 && P.some((p, i) => p.moves.length < want[i]); b++) {
    ...
    for (const i of shuffle([...P.keys()])) {
      ...
      const cands = shuffle(['x', 'y', 'rotation'].filter(k => k !== last[i])).flatMap(k => shuffle([-1, 1]).map(sg => {
        const step = k === 'rotation' ? 90 : k === 'x' ? (1 + rnd(4)) * U : (1 + rnd(3)) * U;
...
function planIntro() {
  ...
  const spread = P => { // every piece really scattered, with air between them
    ...
    return P.every(p => Math.abs(val(p, 'x', 99)) + Math.abs(val(p, 'y', 99)) >= 2 * U) &&
      B.every((a, i) => B.every((b, j) => j <= i || Math.min(a[2], b[2]) - Math.max(a[0], b[0]) < -8 || Math.min(a[3], b[3]) - Math.max(a[1], b[1]) < -8));
  };
